			...
			// e.g a social post, textinput which lets user to enter text, takes persons id as assigned object
				<div style={styles.outerContainer}>
				  	...
				  	<div style={styles.textinputContainer}>
						<form className={styles.root} noValidate autoComplete="off">
							<TextField 
								label="Type your ad_image" // placeholder 
								id="standard-basic" // "filled-basic" / "outlined-basic"
								variant="outlined" // "filled"
								classes={styles.textinput}
								onChange={ (event) => this.setState( prev => ({...prev, ad_image: event.target.value})) }
							/>
						</form>
				  	</div>
					...
					<button style={styles.buttonWithoutBG}
						onClick={ () => {
							...
							let advertisement_object = {
								ad_name: this.state.ad_name,
								ad_image: this.state.ad_image,
							...
							axios.post(utils.baseUrl + '/advertisements/create-advertisement-with-', 
								{
									advertisement_object: advertisement_object,
									_object: _object,
								})
