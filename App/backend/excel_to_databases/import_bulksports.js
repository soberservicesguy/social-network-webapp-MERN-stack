...
const file_name = excel_file || '/home/arsalan/Work_stuff/Full_stack_apps/REACT_APPS/Final_portfolio/content_app/backend/excel_to_databases/all_sports.xlsx'
// const file_name = '/home/arsalan/Work_stuff/Full_stack_apps/REACT_APPS/Final_portfolio/content_app/backend/excel_to_databases/all_sports.xlsx'

const sheet_to_class_mapper = (sheet_name, db_object) => {
	if (sheet_name === 'all_sports'){

		return new Sport(db_object)
...
// finding the user 
	...
	for (let i = 0; i < row_details_list.length; i++) {
		...
		const sport = sheet_to_class_mapper(parent_sheet, {...parent_db_object_dict, _id: new mongoose.Types.ObjectId()})
		...
		// assigning the user
			sport.sport_created_by_user = user_object
			...
			if (related_children.length > 0){
				for (let k = 0; k < related_children.length; k++) {
					...
					for (let l = 0; l < child_rows.length; l++) {
						...
						const related_child = sheet_to_class_mapper(child_sheet, {...child_db_object_dict, sport: sport._id})
						...
						if (child_sheet === 'comments'){
							...
							sport.comments.push(related_child._id)

		
						} else if (child_sheet === 'likes'){

							sport.likes.push(related_child._id)

		
						} else if (child_sheet === 'shares'){

							sport.shares.push(related_child._id)

		
						} else if (child_sheet === 'users'){

							sport.users.push(related_child._id)
