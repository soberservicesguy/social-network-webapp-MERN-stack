...
const file_name = excel_file || '/home/arsalan/Work_stuff/Full_stack_apps/REACT_APPS/Final_portfolio/content_app/backend/excel_to_databases/all_advertisements.xlsx'
// const file_name = '/home/arsalan/Work_stuff/Full_stack_apps/REACT_APPS/Final_portfolio/content_app/backend/excel_to_databases/all_advertisements.xlsx'
...
// finding the user 
	...
	for (let i = 0; i < row_details_list.length; i++) {
		...
		// assigning the user
			page.ad_uploaded_by_user = user_object
				...
				for (let k = 0; k < related_children.length; k++) {
					...
					for (let l = 0; l < child_rows.length; l++) {
						...
						if (child_sheet === 'comments'){
							...
							advertisement.comments.push(related_child._id)

		
						} else if (child_sheet === 'likes'){

							advertisement.likes.push(related_child._id)

		
						} else if (child_sheet === 'shares'){

							advertisement.shares.push(related_child._id)

		
						} else if (child_sheet === 'users'){

							advertisement.users.push(related_child._id)
