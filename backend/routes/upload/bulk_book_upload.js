...
function checkFileTypeForImageAndExcelSheet(file, cb){
	...
	if (file.fieldname === "book_image") { // if uploading resume
...
// Init Upload
const bulk_upload_books = multer({
	storage: bulk_books_storage,
	limits:{fileSize: 200000000}, // 1 mb
	fileFilter: function(req, file, cb){
		checkFileTypeForImageAndExcelSheet(file, cb);
	}
}).fields([
	{ name: 'excel_sheet_for_book', maxCount: 1 }, 
