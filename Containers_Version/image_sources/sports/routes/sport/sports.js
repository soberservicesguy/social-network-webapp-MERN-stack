...
// Init Upload
function upload_sport_by_user(timestamp){
	...
	then(async (sports)=>{
		if (sports){
			...
			let all_sports = await Promise.all(sports.map(async (sport, index)=>{
				...
				// NEW VERSION				
				let sport_image = await get_image_to_display(sport.sport_image, sport.object_files_hosted_at)

				newSport.sport_image = sport_image
