module.exports.isAllowedInteractingWithOthersPosts = (req, res, next) => {
    ...
    // check user privilege
        ...
        let required_privilege = req.user.user_object.privileges.filter(
            function(privilege_object){
                return privilege_object.privilege_name === 'allow_interacting_with_others_post'
            }
        )
        ...
        if (required_privilege){
            ...
            res.status(401).json({ msg: 'isAllowedInteractingWithOthersPosts Privilege missing' });            
